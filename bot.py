...
async def cmp_attribution_dashboard(callback: types.CallbackQuery):
    ...
    hub_intro = (
        "📊 Метки и кампании\n\n"
        "Делаете ссылку вида t.me/<бот>?start=ВАША_МЕТКА\n\n"
        "Правило: части можно разносить через ДВОЙНОЕ подчёркивание __\n"
        "• vk__march → источник vk, кампания march\n"
        "• viber__broadcast__nov → источник, медиум, кампания\n\n"
        "Фиксируется только переход через /start (кнопка «Старт»). Первая метка не перезаписывается.\n"
        "При оплате копируем те же поля в журнал платежей — так видно, сколько денег с какой метки."
    )
